...
class UpCunet2x(nn.Module):
    ...
    def forward_fast_rough(self, x, tile_mode, alpha, pro):
        ...
        if tile_mode < 3:
            ...
            tile_mode = min(min(h0, w0) // 128, int(tile_mode))  # 最小短边为128*128
            if tile_mode < 3:
                return self.forward(x, tile_mode, 1, alpha)
            t2 = tile_mode * 2
            crop_size = (
                ((h0 - 1) // t2 * t2 + t2) // tile_mode,
                ((w0 - 1) // t2 * t2 + t2) // tile_mode,
...
if __name__ == "__main__":
    ...
    input_dir = "%s/input" % root_path
    output_dir = "%s/output" % root_path
    os.makedirs(output_dir, exist_ok=True)

    weight_paths = [
        ("pro-denoise3x-up2x.pth", 2),
        ("pro-denoise3x-up3x.pth", 3),
    ]
            ...
            for cache_mode in [0, 1]:
                for alpha in [0.75, 1, 1.3]:
                    for name in os.listdir(input_dir):
                        ...
                        t1 = time.time()
                        print(
                            f"{prefix} done, Time: {t1 - t0}, tile_mode={tile_mode}, cache_mode={cache_mode}, alpha={alpha}"
                        )

                        opt_path = os.path.join(
                            output_dir,
                            prefix
                            + f"_{scale}x_tile{tile_mode}_cache{cache_mode}_alpha{alpha}.{suffix}",
                        )
                        cv2.imwrite(opt_path, result)
