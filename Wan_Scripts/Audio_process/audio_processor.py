...
class AudioProcessor:
    ...
    def __init__(
        ...
        if audio_separator_model_path is not None:
            try:
                os.makedirs(cache_dir, exist_ok=True)
                self.audio_separator = Separator(
                    output_dir=cache_dir,
                    output_single_stem="vocals",
                    model_file_dir=os.path.dirname(audio_separator_model_path),
                )
                self.audio_separator.load_model(
                    os.path.basename(audio_separator_model_path)
                )
                assert self.audio_separator.model_instance is not None, (
                    "Fail to load audio separate model."
                )
            except Exception as e:
                print(f"Failed to load audio separate model: {e}")
    ...
    def preprocess(self, wav_file: str, clip_length: int = -1, fps: float = 25.0):
        ...
        if clip_length > 0:
            if seq_len > clip_length:
                seq_len = (seq_len // clip_length) * clip_length
                audio_feature = audio_feature[: int(seq_len * (self.sample_rate / fps))]
                audio_length = seq_len
            else:
                pad_length = clip_length - seq_len % clip_length
                if pad_length != clip_length:
                    audio_feature = torch.nn.functional.pad(
                        audio_feature,
                        (0, int(pad_length * (self.sample_rate / fps))),
                        "constant",
                        0.0,
                    )
                    seq_len += pad_length
