...
class SDVideoPipelineRunner:
    ...
    def load_pipeline(self, model_list, textual_inversion_folder, device, lora_list, lora_alphas, controlnet_units):
        # Load models
        model_manager = ModelManager(torch_dtype=torch.float16, device=device)
        model_manager.load_models(model_list)
        if lora_list is not None:
            for lora, alpha in zip(lora_list, lora_alphas):
                model_manager.load_lora(lora, lora_alpha=alpha)
