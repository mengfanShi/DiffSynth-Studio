...
from .sd_motion import SDMotionModelStateDictConverter, SDMotionModel
...
class SDLoRA:
    ...
    def convert_state_dict(self, state_dict, lora_prefix="lora_unet_", alpha=1.0, device="cuda"):
        special_keys = {
            ...
            "attention.blocks": "attention_blocks"
        }
        state_dict_ = {}
    ...
    def add_lora_to_unet(self, unet: SDUNet, state_dict_lora, alpha=1.0, device="cuda", convert_diffusers=True):
    ...
    def add_lora_to_motion_module(self, motion_module: SDMotionModel, state_dict_lora, alpha=1.0, device="cuda", convert_diffusers=True):
        state_dict_motion_module = motion_module.state_dict()
        state_dict_lora = self.convert_state_dict(state_dict_lora, lora_prefix="lora_mm_", alpha=alpha, device=device)
        if convert_diffusers:
            state_dict_lora = SDMotionModelStateDictConverter().from_diffusers(state_dict_lora, mm_v3=True)
        if len(state_dict_lora) > 0:
            print("Adding lora to motion module")
            for name in state_dict_lora:
                state_dict_motion_module[name] += state_dict_lora[name].to(device=device)
            motion_module.load_state_dict(state_dict_motion_module)
