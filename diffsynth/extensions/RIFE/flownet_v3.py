...
def warp(tenInput, tenFlow, device):
    ...
    if k not in backwarp_tenGrid:
        ...
        backwarp_tenGrid[k] = torch.cat(
            [tenHorizontal, tenVertical], 1).to(device, dtype=tenInput.dtype)
...
class IFNet(nn.Module):
    ...
    def forward(self, x, timestep=0.5, scale=[16, 8, 4, 2, 1], training=False, scale_factor=1):
